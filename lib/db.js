...
async function updateRoutine(userId, routineId, fields) {
  if (DATABASE_URL) {
    ...
    const vals = [];
    let idx = 1;
    const fieldMap = { title: "title", meta: "meta", weeks: "weeks", profileSnapshot: "profile_snapshot" };
    for (const [jsKey, sqlCol] of Object.entries(fieldMap)) {
      if (fields[jsKey] !== undefined) {
        const val = (jsKey === "weeks" || jsKey === "profileSnapshot") ? JSON.stringify(fields[jsKey]) : fields[jsKey];
        sets.push(`${sqlCol} = $${idx}`);
        vals.push(val);
        idx += 1;
      }
