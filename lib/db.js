...
async function init() {
  if (DATABASE_URL) {
    ...
    // Add completions column if missing
    await pg.query(`
      ALTER TABLE routines ADD COLUMN IF NOT EXISTS completions JSONB NOT NULL DEFAULT '{}'
    `);
...
async function updateRoutine(userId, routineId, fields) {
  if (DATABASE_URL) {
    const pg = getPool();
    const sets = [];
    const vals = [];
    let idx = 1;
    const fieldMap = { title: "title", meta: "meta", weeks: "weeks", profileSnapshot: "profile_snapshot", completions: "completions" };
    for (const [jsKey, sqlCol] of Object.entries(fieldMap)) {
      if (fields[jsKey] !== undefined) {
        const val = (jsKey === "weeks" || jsKey === "profileSnapshot" || jsKey === "completions") ? JSON.stringify(fields[jsKey]) : fields[jsKey];
        sets.push(`${sqlCol} = $${idx}`);
        vals.push(val);
        idx += 1;
      }
    ...
    vals.push(routineId, userId);
    const result = await pg.query(
      `UPDATE routines SET ${sets.join(", ")} WHERE id = $${idx} AND user_id = $${idx + 1} RETURNING *`,
      vals
    );
    return result.rows.length > 0 ? routineFromRow(result.rows[0]) : null;
  }
  const data = readFile();
  const user = data.users.find((u) => u.id === userId);
  if (!user) return null;
...
async function toggleSessionCompletion(userId, routineId, key) {
  if (DATABASE_URL) {
    const pg = getPool();
    const result = await pg.query(
      "SELECT completions FROM routines WHERE id = $1 AND user_id = $2",
      [routineId, userId]
    );
    if (result.rows.length === 0) return null;
    const completions = result.rows[0].completions || {};
    if (completions[key]) {
      delete completions[key];
    } else {
      completions[key] = new Date().toISOString();
    }
    const updated = await pg.query(
      "UPDATE routines SET completions = $1 WHERE id = $2 AND user_id = $3 RETURNING *",
      [JSON.stringify(completions), routineId, userId]
    );
    return routineFromRow(updated.rows[0]);
  }
  const data = readFile();
  const user = data.users.find((u) => u.id === userId);
  if (!user) return null;
  ...
  if (routine.completions[key]) {
    delete routine.completions[key];
  } else {
    routine.completions[key] = new Date().toISOString();
  }
  writeFile(data);
  return routine;
}
...
async function getUserStats(userId) {
  ...
  for (const routine of routines) {
    ...
    for (const [, timestamp] of Object.entries(completions)) {
      completedSessions += 1;
      completionDates.push(timestamp.slice(0, 10));
      const snapshot = routine.profileSnapshot || {};
      const weaknessArr = snapshot.weaknesses && snapshot.weaknesses.length > 0
        ? snapshot.weaknesses : [snapshot.weakness || "Unknown"];
      for (const w of weaknessArr) {
  ...
  streak = 0;
  const sortedAsc = [...new Set(completionDates)].sort();
  ...
  return {
    ...
    currentStreak,
    longestStreak,
    weaknessCoverage: weaknessCounts
  };
}
